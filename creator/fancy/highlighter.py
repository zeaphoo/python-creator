...
class GenericHighlighter(QtGui.QSyntaxHighlighter):
    def __init__(self, parent, lang='python'):
        ...
        lang_scheme = SYNTAXES+ lang + '_scheme.xml'
        self.ctx_rules, self.words_rules, self.word_rules = readRulesFromXML(lang_scheme)
    ...
    def changeRules(self, lang='python'):
        lang_scheme = SYNTAXES+ lang + '_scheme.xml'
        self.ctx_rules, self.words_rules, self.word_rules = readRulesFromXML(lang_scheme)
        self.cmt_rules = [f for f in self.word_rules if f.isComment]
        self.normal_rules = [f for f in self.word_rules if not f.isComment]
